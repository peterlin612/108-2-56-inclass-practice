...
course_Update[[2]][[2]] <- "選"
course_Update[[1]] <- as.logical(course[[1]])
str(course_Update)
```
...
```{r ans32}
# Pikachu_stats
Pikachu_stats <- Pikachu
Pikachu_stats[[4]][c(1,2,3)] <- as.numeric(Pikachu_stats[[4]][c(1,2,3)])
str(Pikachu_stats)
```
...
```{r ans34}
# Gamemaster
Gamemaster[[1]][[731]] <- Pikachu
Mimikyu <- Gamemaster[[1]][[731]]
Mimikyu[[1]] <- as.integer(778)
Mimikyu[[2]] <- "Mimikyu"
Mimikyu[[3]] <- "mimikyu"
Mimikyu[[5]] <- c("ghost","fairy")
Mimikyu[c(6,7,8)] <- Jigglypuff[c(6,7,8)]
str(Mimikyu)
```
