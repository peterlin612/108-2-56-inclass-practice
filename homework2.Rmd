...
get1 <- course[2]
str(get1)
get1$course -> get1_layer2
get1_layer2[[4]]
time_place <- get1_layer2[[4]]
print(time_place)
```
...
```{r ans22}
# course_update
course_update <- list(approve=T,course=list(coursename="會計學",type="選",credits=3L,time_place=list(weekday='星期三',time_slots=c(7L,8L,9L),classroom="商2F05")))
str(course_update)
```
...
```{r ans23}
# course_update2
course_update2 <- list(approve=T,course=list(teacher="紀學檜老師",coursename="會計學",type="選",credits=3L,time_place=list(weekday='星期三',time_slots=c(7L,8L,9L),classroom="商2F05")))
str(course_update2)
```
...
Pikachu_layer1[["baseStats"]] -> Pikachu_stats
str(Pikachu_stats)
```
